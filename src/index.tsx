...
export const StackOverflowBadge = ({ id, card, logo }: Props) => {
  const [user, setUser] = useState<any>(null)

  useEffect(() => {
    axios
      .get(`https://api.stackexchange.com/2.2/users/${id}`, {
        params: {
          site: 'stackoverflow'
        }
      })
  ...
  return user !== null ? (
    <a
      className={'StackOverflowBadge ' + (card ? 'card' : '')}
      href={`https://stackoverflow.com/users/${id}?tab=profile`}
      target='_blank'
    ...
    <div className={'StackOverflowBadge ' + (card ? 'card' : '')}>
      {logo && (
        <svg
          className='logo'
          width='26'
